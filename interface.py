...
class MdUpdate:  # Data of a tick
    def __init__(
            self,
            md: pd.core.frame.DataFrame,
            timestamp: float,
            delta_t: float):

        self.time_to_next_tick = delta_t

        self.trades = list(map(lambda i: AnonTrade(timestamp,
                                                   md['aggro_side'].values[i],
                                                   md['size'].values[i],
                                                   md['price'].values[i]),
                               range(len(md['size']))))
        self.recieve_trades_latency = list(map(
            lambda i: (-md['exchange_ts'] + md['receive_ts_x']).values[i], range(len(md['size']))))

        self.orderbook = OrderbookSnapshotUpdate()
        self.orderbook.timestamp = timestamp

        self.orderbook.asks = list(map(lambda i: ((md[pair +
                                                      ':Binance:LinearPerpetual_ask_price_' +
                                                      str(i)]).values[0], (md[pair +
                                                                              ':Binance:LinearPerpetual_ask_vol_' +
                                                                              str(i)]).values[0]), range(10)))

        self.orderbook.bids = list(map(lambda i: ((md[pair +
                                                      ':Binance:LinearPerpetual_bid_price_' +
                                                      str(i)]).values[0], (md[pair +
                                                                              ':Binance:LinearPerpetual_bid_vol_' +
                                                                              str(i)]).values[0]), range(10)))
        self.recieve_orderbook_latency = (
            -md['exchange_ts'] + md['receive_ts_y']).values[0]
...
class Strategy:
                    ...
                    if (bool(random.randint(0, 1))):
                        id = sim.place_order(
                            ...
                            random.uniform(
                                1e-6,
                                self.max_size),
                            (self.md_update.orderbook.asks[0])[0])
                    else:
                        id = sim.place_order(
                            'BID',
                            random.uniform(
                                1e-6,
                                self.max_size),
                            (self.md_update.orderbook.bids[0])[0])
    ...
    def update_time_to_cancel(self):
        self.time_to_cancel = list(map(lambda x: (
            x[0] - self.md_update.time_to_next_tick, x[1]), self.time_to_cancel))  # !!!!!!
        id_to_cancel = list(
            (map(
                lambda x: x[0],
                list(
...
def load_md_from_file(path: str) -> list[MdUpdate]:
    ...
    return list(map(lambda i: MdUpdate(
        md[md['exchange_ts'] == ts[i]], ts[i], delta_t[i]), range(10**3)))
...
class Sim:
    ...
    def prepare_orders(self):
        ...
        temp_list = list(
            filter(
                lambda x: (
                    x[0] +
                    self.current_md.recieve_orderbook_latency <= 0) and (
                    ((x[1].side == 'ASK') and (
                        x[1].price >= (
                            self.current_md.orderbook.asks[0])[0])) or (
                        (x[1].side == 'BID') and (
                            x[1].price <= (
                                self.current_md.orderbook.bids[0])[0]))),
                self.not_placed_orders))
        # non placed orders -> placed
        self.placed_orders.extend(list(map(lambda x: x[1], temp_list)))

        self.disabled_orders.extend(
            list(
                map(
                    lambda x: x[0],
                    list(
                        filter(
                            lambda x: x[0] +
                            self.current_md.recieve_orderbook_latency <= 0 and not (
                                (((x[1].side == 'ASK') and (
                                    x[1].price >= (
                                        self.current_md.orderbook.asks[0])[0])) or (
                                    (x[1].side == 'BID') and (
        ...
        self.not_placed_orders = list(
            filter(
                lambda x: not (
                    x[1].id in self.disabled_orders),
                self.not_placed_orders))  # clear
        # from disabled orders
        self.not_placed_orders = list(
            filter(
                lambda x: x[0] +
                self.current_md.recieve_orderbook_latency > 0,
                self.not_placed_orders))  # clear non placed from old orders
            ...
            map(
                ...
                list(
                    filter(
                        lambda x: x[0] +
                        self.current_md.recieve_orderbook_latency <= 0,
                        self.not_canceled_orders))))  # take old 'cancel'
        self.not_canceled_orders = list(
            filter(
                lambda x: x[0] +
                self.current_md.recieve_orderbook_latency > 0,
                self.not_canceled_orders))  # clear not canceled
    ...
    def prepare_trade(self):
        self.trades.extend(list(map(lambda x: x[1], list(
            filter(lambda x: x[0] <= 0, self.not_exec_trades)))))  # not_exec -> exec
        self.not_exec_trades = list(
            filter(
                lambda x: x[0] > 0,
                self.not_exec_trades))  # clear not_exec
        ...
        for order in self.placed_orders:
            ...
            for trade in anon_trades:
                if ((order.side == 'ASK' and trade.price >= order.price) or
                    ...
                    self.not_exec_trades.append(
                        (self.md_latency + self.current_md.recieve_trades_latency[count], new_trade))
    ...
    def update_latency(self):
        self.not_placed_orders = list(map(lambda x: (
            x[0] - self.current_md.time_to_next_tick, x[1]), self.not_placed_orders))
        self.not_exec_trades = list(map(lambda x: (
            x[0] - self.current_md.time_to_next_tick, x[1]), self.not_exec_trades))
        self.not_canceled_orders = list(map(lambda x: (
            x[0] - self.current_md.time_to_next_tick, x[1]), self.not_canceled_orders))
    ...
    # ордер отправлен, еще не дошел до биржи
    not_placed_orders: list[tuple[float, Order]] = []
    not_canceled_orders: list[tuple[float, float]] = []
    # Создали трейд, еще не отправили в стратегию
    not_exec_trades: list[tuple[float, OwnTrade]] = []
