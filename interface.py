...
class MdUpdate:  # Data of a tick
    def __init__(self, md: pd.core.frame.DataFrame, timestamp: float):     
...
class Strategy:
    ...
    def run(self, sim: "Sim"):
        while True:
            try:
                md_update, placed_orders, trades, disabled_orders, non_placed_orders, non_canceled_orders = sim.tick()
                ...
                if ( abs(self.current_pos) <= self.max_pos):
                    if (bool(random.randint(0, 1))):
                        id = sim.place_order('ASK', random.uniform(1e-6, self.max_size), (md_update.orderbook.asks[0])[0])
                    else:
                        id = sim.place_order('BID', random.uniform(1e-6, self.max_size), (md_update.orderbook.bids[0])[0])
    ...
    def update_time_to_cancel(self):
        self.time_to_cancel = list(map(lambda x: (x[0]-1, x[1]), self.time_to_cancel))
...
def load_md_from_file(path: str) -> list[MdUpdate]:
    ...
    return list(map(lambda i: MdUpdate(md[md['exchange_ts'] == ts[i]], ts[i]),
                range(10**3)))
...
class Sim:
    ...
    def prepare_orders(self):
        ...
        temp_list = list(filter( lambda x: ( x[0] <= 0 ) and 
        ...
        self.disabled_orders.extend(list(map( lambda x: x[0], list(filter(lambda x: x[0] <= 0 and
        ...
        self.not_placed_orders = list(filter(lambda x: x[0] > 0, self.not_placed_orders)) #clear non placed from old orders
    
        temp_list_id = list(map( lambda x: x[1], list(filter(lambda x: x[0] <= 0, self.not_canceled_orders )))) #take old 'cancel'
        self.not_canceled_orders = list(filter(lambda x: x[0] > 0, self.not_canceled_orders)) #clear not canceled  
        ...
        for order in self.placed_orders:
            ...
            for trade in anon_trades:
                if  ((order.side == 'ASK' and trade.price >= order.price) or 
                    ...
                    self.not_exec_trades.append((self.md_latency, new_trade))
    ...
    def update_latency(self):
        self.not_placed_orders = list(map(lambda x: (x[0]-1, x[1]), self.not_placed_orders))
        self.not_exec_trades = list(map(lambda x: (x[0]-1, x[1]), self.not_exec_trades))
        self.not_canceled_orders = list(map(lambda x: (x[0]-1, x[1]), self.not_canceled_orders))
